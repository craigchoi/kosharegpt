fetch('https://kosharegpt.com/api/auth/csrf').then(async (res) => {
  ...
  document.querySelector('#csrfToken-naver').value = csrf;
});

fetch('https://kosharegpt.com/api/auth/session').then(async (res) => {
  const json = await res.json();
